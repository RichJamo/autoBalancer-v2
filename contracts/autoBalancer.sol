...
contract autoBalancer is KeeperCompatibleInterface {
    ...
    address public constant SAND_ADDRESS = 0xBbba073C31bF03b8ACf7c28EF0738DeCF3695683;
    ...
    address public constant SAND_USD_ORACLE = 0x3D49406EDd4D52Fb7FFd25485f32E073b529C924;
    ...
    function depositUserFunds (uint256 amount_) public  {
        ...
        uint256 SAND_balanceInUSD = getUSDTokenBalanceOf(SAND_ADDRESS, SAND_USD_ORACLE, 18);
        ...
        uint256 Total_in_USD = WMATIC_balanceInUSD + SAND_balanceInUSD + WETH_balanceInUSD + WBTC_balanceInUSD;
        
        approve_spending(USDC_ADDRESS, QUICKSWAP_ROUTER, amount_);

        if (Total_in_USD > 0) {
            swapProportionately(WMATIC_balanceInUSD, SAND_balanceInUSD, WETH_balanceInUSD, WBTC_balanceInUSD, Total_in_USD, amount_);
        } else {
    ...
    function swapProportionately(uint256 WMATIC_amount, uint256 SAND_amount, uint256 WETH_amount, uint256 WBTC_amount, uint256 totalUSDAmount, uint256 depositAmount) public {
        ...
        swap(SAND_share, uint256(0), USDCToSANDPath, address(this), 99999999999);
    ...
    function swapIntoFourEqualParts(uint256 amount) public {
        ...
        swap(amount / 4, uint256(0), USDCToSANDPath, address(this), 99999999999);
    ...
    function withdrawUserFunds(address user) public {
        ...
        uint256 SAND_amount = userNumberOfShares[user] * (tokenBalanceOf(SAND_ADDRESS, address(this))) / (totalNumberOfShares);
